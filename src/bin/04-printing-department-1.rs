...
fn run() -> Result<String, String> {
    ...
    let adjacent_positions = [Point { x: -1, y: -1 }, Point { x: 0, y: -1 }, Point { x: 1, y: -1 }, Point { x: -1, y: 0 }, Point { x: 1, y: 0 }, Point { x: -1, y: 1 }, Point { x: 0, y: 1 }, Point { x: 1, y: 1 }];
    let mut found = 0;
    let width = input.len();
    for x in 0..width {
        let line = &input[x];
                ...
                Item::RollOfPaper => {
                    ...
                    for pos in adjacent_positions.iter() {
                        match get_ajacent_point(x, y, pos, width, height) {
                            Some(point) => {
                                match input[point.x as usize][point.y as usize] {
                                    Item::RollOfPaper => { adjacent_rolls_of_paper += 1; }
                                    _ => {}
                                }
                            }
                            _ => {}
    ...
    return Ok(format!("{}", found));
}

fn get_ajacent_point(x: usize, y: usize, pos: &Point, width: usize, height: usize) -> Option<Point> {
    ...
    if adjacent_x < 0 || adjacent_y < 0 || adjacent_x >= width as i32 || adjacent_y >= height as i32 {
        ...
        Some(Point { x: adjacent_x, y: adjacent_y  })
    }
}
